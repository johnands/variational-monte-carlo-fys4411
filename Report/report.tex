...
In three dimension $F(x)$ is called the \textit{drift vector} ${\bf F}({\bf x})$. This quantity can be found from the
Fokker-Planck equation
\begin{equation}
 \frac{\partial P}{\partial t} = \sum_i D \frac{\partial}{\partial {\bf x}_i}
 \left( {\bf x}_i - {\bf F}_i \right) P({\bf x}, t)
\end{equation}
which describes the process of isotropic diffusion
by a time-dependent probability density $P({\bf x}, t)$ with a drift ${\bf F}$.
The convergence to a stationary probability density can be obtained by settinf the left hand side to zero. 
...
\\ The Monte Carlo method with the Metropolis algoritgh can be seen as isotropic diffusion by a time-dependent probability density, 
with or without a drift, corresponding to brute force
